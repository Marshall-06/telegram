...
import { cancelWithdrawal, completeWithdrawal, findWithdrawalByShortId, formatWithdrawalList, getPendingWithdrawals, } from "../../services/withdrawal.service.js";
...
export async function showAdminPanel(ctx) {
    ...
    await ctx.reply(t(locale, "adminPanel", { botUsername: botInfo.username ?? "Vanguard5_bot" }), { parse_mode: "Markdown" });
}
export function registerAdminCommands(bot) {
    ...
    bot.command("admin_cekim_tamam", async (ctx) => {
        ...
        try {
            ...
            try {
                await ctx.api.sendMessage(Number(completed.user.telegramId), t(userLocale, "withdrawCompletedUser", {
                    amount: formatAmount(completed.amount),
                }));
            }
            catch {
    ...
    bot.command("admin_cekim_iptal", async (ctx) => {
        ...
        try {
            ...
            try {
                await ctx.api.sendMessage(Number(cancelled.user.telegramId), t(userLocale, "withdrawCancelledUser", {
                    amount: formatAmount(cancelled.amount),
                }));
            }
            catch {
...
export async function notifyAdminsPendingWithdrawals(bot) {
    const admins = getAdminTelegramIds();
    if (admins.length === 0)
        return;
    const pending = await getPendingWithdrawals();
