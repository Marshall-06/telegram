...
} from "../helpers/locale-context.js";
import {
  cancelMenuKeyboard,
...
function formatBusinessError(locale: Locale, error: unknown): string {
...
async function ensureUserReady(ctx: BotContext): Promise<boolean> {
  if (!(await ensureLanguageSelected(ctx))) return false;
  if (!(await ensureOnboardingComplete(ctx))) return false;
  return true;
}
...
export function registerCommands(bot: Bot<BotContext>): void {
  ...
  bot.command("about", async (ctx) => {
    if (!(await ensureUserReady(ctx))) return;
...
async function handleFlowInput(ctx: BotContext): Promise<void> {
  const locale = await resolveContextLocale(ctx);
  const dbUser = await getDbUser(ctx);
  const text = ctx.message?.text;
  if (!dbUser || !ctx.from || !text) return;
...
async function startDeposit(ctx: BotContext): Promise<void> {
  if (!(await ensureUserReady(ctx))) return;
  const locale = await resolveContextLocale(ctx);
...
async function startWithdraw(ctx: BotContext): Promise<void> {
  if (!(await ensureUserReady(ctx))) return;
  const locale = await resolveContextLocale(ctx);
  const dbUser = await getDbUser(ctx);
  if (!dbUser) return;
