...
import { parseLocale, t, type Locale } from "../i18n/index.js";
...
export async function promptStartLanguageSelection(
  ctx: BotContext,
  name: string,
): Promise<void> {
...
export async function promptLanguageSelection(ctx: BotContext): Promise<void> {
  await ctx.reply(t("en", "languagePrompt"), {
    reply_markup: languageInlineKeyboard(),
  });
}

export async function applyLanguage(ctx: BotContext, locale: Locale): Promise<void> {
...
export async function ensureLanguageSelected(ctx: BotContext): Promise<boolean> {
  const dbUser = await getDbUser(ctx);
  if (!dbUser) return false;

  if (dbUser.languageConfirmed) return true;
