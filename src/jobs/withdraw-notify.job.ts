...
export function startWithdrawNotifyJob(): void {
  const hour = env.WITHDRAW_NOTIFY_HOUR;
  const days = parseWithdrawDays(env.WITHDRAW_DAYS);

  cron.schedule(`0 ${hour} * * *`, async () => {
    const today = new Date();
    if (!isWithdrawDay(today, days)) return;

    try {
      const bot = createBot();
      await notifyAdminsPendingWithdrawals(bot);
      logger.info(
        { day: today.getDate(), pendingNotify: true },
        "Cekim gunu listesi adminlere gonderildi",
      );
    } catch (error) {
  ...
  logger.info(
    { days: env.WITHDRAW_DAYS, hour },
    "Cekim gunu bildirim job'u aktif",
  );
}
