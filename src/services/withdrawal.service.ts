...
type WithdrawalWithUser = Withdrawal & { user: User };

export async function getPendingWithdrawTotal(userId: string): Promise<Decimal> {
...
export async function getAvailableBalance(userId: string): Promise<Decimal> {
...
export async function getPendingWithdrawals(): Promise<WithdrawalWithUser[]> {
...
export function formatWithdrawalList(items: WithdrawalWithUser[], locale: Locale = "tr"): string {
  if (items.length === 0) {
    return locale === "ru"
  ...
  return (
    `*CEKIM LISTESI* (${items.length} talep)\n` +
...
export async function cancelWithdrawal(withdrawalId: string, adminNote?: string) {
  const withdrawal = await prisma.withdrawal.findUnique({
    where: { id: withdrawalId },
  });
...
export async function findWithdrawalByShortId(shortId: string) {
  const pending = await getPendingWithdrawals();
  return pending.find(
    (w) => w.id.endsWith(shortId) || w.id === shortId,
  );
}
